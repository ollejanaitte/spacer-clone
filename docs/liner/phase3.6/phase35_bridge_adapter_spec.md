...
3. Phase 3.5 の既存 save path に `liner.domainDraft` として保存する。
...
ログはユーザーが「どの PDF ページのどの行列から Phase 3.5 draft ができたか」を追跡できることを目的にする。Phase 3.5 draft の payload を肥大化させないため、詳細 sourceRef は log 側に集約する。
