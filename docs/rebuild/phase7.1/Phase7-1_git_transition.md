...
- **注意**: これは「確認可能な最初の文書化」であり、「方針の絶対的起源」ではない（それ以前に口頭・別文書で方針が存在した可能性は否定できない）。

### ユーザー明示指示の証拠（C）

- **確認したrepository文書・取得済みGit履歴内では見つからなかった**。Phase 2-00監査書・Phase 2 design書に「ユーザーが旧/pro/liner UIを復活させないよう指示した」という記録・引用は無い。

### AI/実装側判断だった可能性（D）

- Phase 2-00監査は実装側（rebuild phase担当）の設計判断として文書化されている。
- 「新システムの正規導線」をModule Core（/app）とする方針は、rebuildアーキテクチャ（Project Data Core / Module Core）に基づく実装側判断と**推定**される（合理的推定ではあるが、これは「事実A」ではなく推定）。

### 証拠不足でUNKNOWN（E）

- 旧/pro/liner UIを復活させない方針の「ユーザー明示指示」の有無は、確認した範囲では**証拠が無い**。ユーザー指示があった可能性は否定できないが、repository文書・GitHub上で裏付けられない。
