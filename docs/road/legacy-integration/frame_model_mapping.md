...
> The body-level "source of truth" statement is limited to the current legacy direct generator. It is not target Road-to-Frame mapping, package, or apply authority. `RoadDesignDocument` and `BridgeFrameAnalysisDocument` remain separate; use the [transfer crosswalk](../../transfer/contract-index.md) and [formal contract](../../planning/stage6-10/road_to_frame_contract.md).
<!-- DOC-AUTHORITY:END -->
...
- Geometry computation (see [geometry_core.md](../design/geometry_core.md)).
...
- Frame model uses right-handed global coordinates consistent with [coordinate_system_policy.md](../design/coordinate_system_policy.md).
- SI units match [docs/frame/contracts/04_input_schema.md](../../frame/contracts/04_input_schema.md).
...
Frame analysis uses member local axes for stiffness, releases, and loads. Convention aligns with [coordinate_system_policy.md](../design/coordinate_system_policy.md) and existing `orientationVector` in `project.schema.json`.
...
**DOF flags:** Per [docs/frame/contracts/04_input_schema.md](../../frame/contracts/04_input_schema.md) support schema (`ux`, `uy`, `uz`, `rx`, `ry`, `rz` booleans).
...
When station equations create duplicate displayed stations, ordering follows `physicalDistance` from station table — geometry core must emit consistent indices ([geometry_core.md](../design/geometry_core.md) §7).
...
Full fixture: `examples/liner/gc-06-intermediate.json` → `examples/liner/gc-06-project.generated.json` ([test_plan_geometry.md](../verification/test_plan_geometry.md)).
...
- [coordinate_system_policy.md](../design/coordinate_system_policy.md)
