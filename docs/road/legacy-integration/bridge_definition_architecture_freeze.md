...
> This architecture freeze governs only the retained Phase 4.5 legacy path. It does not freeze the Stage 6-10 target architecture: `BridgeDefinition` is not the target package or shared source of truth, and direct output switching is not target apply authority. Use the [transfer crosswalk](../../transfer/contract-index.md) and [formal contract](../../planning/stage6-10/road_to_frame_contract.md).
<!-- DOC-AUTHORITY:END -->
...
> - [bridge-domain-model.md](../../frame/modeler/bridge-domain-model.md) — BridgeProject（Wizard 入力モデル）
