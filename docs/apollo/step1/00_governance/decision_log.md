...
| DEC-S1-0005 | 2026-07-27 | P03 inventory: classify capabilities by **operational vs infrastructure** layers; legacy `ProjectModel` remains operational Frame SoR while target contract documents are validated infrastructure | Repo evidence shows working analysis/viewer on ProjectModel parallel to v0.1 BFAD/RDD/TR schemas and IF3 modules; conflating schema presence with product SoR would overstate readiness | ACCEPTED | P03 (pending merge) |
| DEC-S1-0006 | 2026-07-27 | P03 documents **IF3 client binding gap** as current limitation: `apiClient.runAnalysis` omits `if3` metadata required for authoritative export gates | Backend normalizer and tests expect binding; default UI path fail-closed per IF3 consumer contracts; not a design change in P03 | ACCEPTED | P03 (pending merge) |
