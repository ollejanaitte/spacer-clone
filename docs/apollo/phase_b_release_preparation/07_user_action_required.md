...
| UA-P5-01 | PB-RC-A | A（主桁断面諸量・純幾何）の独立表計算/手計算結果の作成・確認・署名（06_first_numeric_release_candidate.md §6。対称・非対称 I 断面の断面積・図心・断面二次モーメント・断面係数。丸め・許容誤差を凍結）。作業票・fixture・承認欄の正本: `docs/apollo/step1_numeric_core/evidence_golden/`（GOLD-SP-001/002 = GOLD-MG-003S/003A）。Cursor 計算値を独立 Golden にしない。 | 独立表計算 or 手計算 + evidence_golden パッケージ | 高 | 候補選定済み・独立計算/署名待ち（Step 1-A PATH B パッケージ作成済み） | P5 / Step 1-A |
