...
| Windows Apollo viewer controls / fallback / diagnostics | `14_axis_camera_main_viewer_bug_report.md`, `15_windows_viewer_controls_fallback_diagnostics.md` | current fix branch | `frontend/src/viewer/*`, `desktop/electron/preload.ts`, `frontend/src/desktop/spacerDesktop.d.ts` | runtime diagnostics tests + browser smoke + Electron startup smoke | Windows operator-facing controls and disclosure |
| Linux launcher lifecycle / background cleanup | `11_persistence_reload_electron_design.md`, `16_linux_electron_launcher_smoke_solid_verification.md` | fix PR-A | `start-ubuntu.sh`, `desktop/electron/main.ts` | foreground/nohup failure-path launch checks + port/process inspection | no orphan backend on wrapper exit |
| Linux Apollo Electron interactive smoke | `15_windows_viewer_controls_fallback_diagnostics.md`, `16_linux_electron_launcher_smoke_solid_verification.md` | fix PR-B | `frontend/scripts/verifyApolloElectron.mjs`, `frontend/src/viewer/Viewer3D.tsx` | `xvfb-run` Electron smoke + diagnostics artifact capture | attach / route / sample / fallback / cleanup |
