...
| PR-3.6-1b | プロジェクト/橋梁 CRUD + ファイル I/O | 独立 JSON 保存、読み込み、一覧、名前付き途中保存 / 保存済み一覧 / 再開 | なし |
...
- named draft save / saved JSON list / resume by lastEditedStep
...
- 名前付き JSON 保存が任意タイミングで可能。
- 保存済み JSON 一覧から再開でき、lastEditedStep に復帰する。
- バリデーション error 状態でも Draft 保存できる。
