...
| `npm run test -- src/liner` | **PASS** | P2-4 observed 13 files, 61 tests |
...
| Circular arc, signed curvature (GC-02) | **PASS** | — | Midpoint y uses corrected value `17.037087`; matches [test_plan_geometry.md](../../road/verification/test_plan_geometry.md) |
...
| README still says "design skeletons only" | **FAIL** (P2-0 corrected) | LOW | [README.md](../../liner/README.md) status table was not updated for implementation phase at gate time |
...
| ID | Area | Description |
| --- | --- | --- |
| L-01 | Documentation | [README.md](../../liner/README.md) described folder as design-only at gate time; P2-0 updates the index to reflect implemented liner artifacts. |
| L-02 | Documentation | [calculation_pipeline.md](../../road/design/calculation_pipeline.md) pre-implementation checkbox stale for GC-06 integration. |
...
- UI implementation through P2-4 respects layer boundaries; React setup UI uses adapter helpers and centralized i18n.
...
5. Populate `messageKey` on core diagnostics per [error_handling.md](../../road/design/error_handling.md) and add matching `liner.errors.*` keys in i18n when UI starts.
...
| GC-08 through GC-10 clothoid baselines | **Fail-closed by P1-D04** | No independent numeric baselines are committed. Phase 1 keeps clothoid production shipment blocked by `isPhase0ClothoidApproximation()` and adds validation tests for invalid clothoid inputs. |
| UI/save-load/legacy integration | **Covered by P1-D05** | Existing `/pro/liner/*` routes, setup/preview/mapping review, canonical `project.liner.domainDraft` write/read, legacy read-old/write-target tests, and a focused browser save/load scenario provide Phase 1 reachability evidence. |
| Final Phase 1 verification | **Passed by P1-D06** | See [phase1_final_verification.md](phase1_final_verification.md) for the acceptance matrix and final validation command evidence. |
