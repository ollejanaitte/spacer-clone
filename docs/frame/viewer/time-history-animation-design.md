...
result block lives in [result-schema.md](../contracts/result-schema.md); the result
view layer lives in [result-visualization.md](result-visualization.md);
the time history UI is described in
[time-history-ui-design.md](../ui/time-history-ui-design.md).
...
- Document the manual smoke test procedure in
  `docs/frame/verification/time-history-animation-manual-smoke-test.md`.
