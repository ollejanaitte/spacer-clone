...
この文書はJIP-SPACER操作マニュアル由来の広範な要求抽出であり、実装指示書ではない。

MVP実装範囲は `docs/02_mvp_scope.md` を最優先する。この文書に記載された機能であっても、`docs/02_mvp_scope.md` でMVP外とされたものは実装しない。

Codex実装エージェントは、この文書をMVP拡張の根拠にしてはならない。MVP範囲の判断に迷う場合は `docs/02_mvp_scope.md`、品質判断では `docs/12_quality_gate.md` を優先する。

## 目的

JIP-SPACERを参考にした独自の任意形立体骨組解析システムを検討するため、操作マニュアルから広範な候補機能を抽出し、優先度を `Must` / `Should` / `Could` に分類する。

本書は参考資料であり、MVP実装の直接の指示書ではない。
