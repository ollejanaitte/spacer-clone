...
exports.DEFAULT_STATION_FORMAT_OPTIONS = void 0;
exports.formatStationNoPlus = formatStationNoPlus;
exports.parseStationNoPlus = parseStationNoPlus;
exports.parseStationInput = parseStationInput;
exports.formatStationDisplay = formatStationDisplay;
exports.formatStationPlanNotation = formatStationPlanNotation;
exports.isMajorStationDistance = isMajorStationDistance;
exports.DEFAULT_STATION_FORMAT_OPTIONS = {
    interval: 100,
    precision: 3,
    prefix: "No.",
};
 ...
 */
function formatStationNoPlus(meters, options) {
    if (!Number.isFinite(meters))
        return null;
    const opts = { ...exports.DEFAULT_STATION_FORMAT_OPTIONS, ...options };
 ...
 * パース失敗時は null。
 */
function parseStationNoPlus(text, options) {
    ...
    if (noPart.length === 0 || remPart.length === 0)
        return null;
    if (!/^\d+$/.test(noPart))
        return null;
    if (!/^\d+(\.\d+)?$/.test(remPart))
    ...
    if (!Number.isFinite(no) || !Number.isFinite(rem))
        return null;
    const value = no * opts.interval + rem;
    return negative ? -value : value;
}
function parseStationInput(text, options) {
    if (typeof text !== "string")
 ...
 */
function formatStationDisplay(meters) {
 ...
 */
function formatStationPlanNotation(meters, majorInterval = 100) {
    if (!Number.isFinite(meters)) {
        return "";
    }
    const negative = meters < 0;
...
function isMajorStationDistance(meters, majorInterval = 100) {
    if (!Number.isFinite(meters)) {
        return false;
    }
