...
test.describe("P4-D03 HAUNCH utilities smoke", () => {
  test.beforeAll(() => {
    mkdirSync(OUT_DIR, { recursive: true });
  });

  test("HAUNCH editor visible and definitions persist in geometry extension", async ({ page }) => {
    ...
    await expect(page.locator("[data-testid=haunch-definition-editor]")).toBeVisible();

    await page.locator("[data-testid=haunch-definition-add]").click();
    const definitionRow = page.locator("[data-testid^=haunch-definition-row-]").first();
    ...
    const haunchDefinitions = (geometryDraft?.haunchDefinitions ?? []) as HaunchDefinitionSaved[];
    expect(haunchDefinitions.some((definition) => definition.id === definitionId)).toBe(true);
    expect(geometryDraft).not.toHaveProperty("haunchResults");
    expect(saved.liner?.roadDesignDocument).not.toHaveProperty("haunchResults");
  });
});
