...
type QuakePickerProps = {
  onShake: (preset: EarthquakePresetId) => void;
  onBack: () => void;
};
  ...
  return (
    <div className="level0-quake-picker">
        ...
        {(Object.keys(EARTHQUAKE_PRESETS) as EarthquakePresetId[]).map((id) => {
          ...
          return (
            <button
              key={id}
              type="button"
              className={`level0-preset-card ${selected === id ? "selected" : ""}`}
      ...
      <div className="level0-picker-actions">
        <button type="button" className="level0-back-button" onClick={onBack}>
          {text.backHome}
        </button>
        <button
          type="button"
          className="level0-shake-button"
          onClick={handleShake}
          disabled={!selected}
        >
          {text.shakeButton}
