...
function projectEndpointKey(
  nodeIKey: string,
  nodeJKey: string,
  nodeMatchMapByLeft: Map<string, string>,
): string | undefined {
  const mappedI = nodeMatchMapByLeft.get(nodeIKey);
  const mappedJ = nodeMatchMapByLeft.get(nodeJKey);
  if (!mappedI || !mappedJ) return undefined;
  return canonicalPair(mappedI, mappedJ);
}

function nodalTargetKey(nodeKey: string, loadCaseSemanticKey: string): string {
  return `${nodeKey}:${loadCaseSemanticKey}`;
}

function memberTargetKey(
  endpointKey: string,
...
function nodalCrossModelKey(
  load: NormalizedNodalLoad,
  nodeKey: string,
): string {
  return nodalTargetKey(nodeKey, load.loadCaseSemanticKey);
}

function memberCrossModelKey(
  load: NormalizedMemberLoad,
  endpointKey: string,
): string {
  return memberTargetKey(endpointKey, load.loadCaseSemanticKey, load.coordinateSystem);
}

function nodalMagnitude(vector: NodalLoadVector): number {
  return Math.sqrt(
    vector.fx * vector.fx
    + vector.fy * vector.fy
    + vector.fz * vector.fz
...
function memberMagnitude(vector: MemberLoadVector): number {
  return Math.sqrt(vector.wx * vector.wx + vector.wy * vector.wy + vector.wz * vector.wz);
}

function totalAppliedLoad(
  nodalLoads: NormalizedNodalLoad[] | undefined,
  memberLoads: NormalizedMemberLoad[] | undefined,
): number {
  let total = 0;
...
function vectorsEqualWithinTolerance(
  left: NodalLoadVector,
  right: NodalLoadVector,
  tolerance: ToleranceBand,
): boolean {
  return (
    nearlyEqual(left.fx, right.fx, tolerance)
    && nearlyEqual(left.fy, right.fy, tolerance)
    && nearlyEqual(left.fz, right.fz, tolerance)
    && nearlyEqual(left.mx, right.mx, tolerance)
    && nearlyEqual(left.my, right.my, tolerance)
    && nearlyEqual(left.mz, right.mz, tolerance)
  );
}

function memberVectorsEqualWithinTolerance(
  left: MemberLoadVector,
  right: MemberLoadVector,
  tolerance: ToleranceBand,
): boolean {
  return (
    nearlyEqual(left.wx, right.wx, tolerance)
    && nearlyEqual(left.wy, right.wy, tolerance)
    && nearlyEqual(left.wz, right.wz, tolerance)
  );
}

function groupByKey<T extends { trace: { sourcePath: string } }>(
  items: T[],
...
function compareGroupedLoads<T extends { trace: { sourcePath: string }; vector: NodalLoadVector | MemberLoadVector }>(
  leftGrouped: Map<string, T[]>,
  rightGrouped: Map<string, T[]>,
  compareVectors: (left: T, right: T) => { equal: boolean; mismatch?: ParityMismatch },
  describeItem: (item: T) => unknown,
): {
  matchedCount: number;
  ...
  for (const key of keys) {
    ...
    if (leftGroup.length === 1 && rightGroup.length === 1) {
      ...
      if (!comparison.equal) {
        ...
        if (comparison.mismatch) mismatches.push(comparison.mismatch);
      } else {
        matchedCount += 1;
      }
    }
...
export function compareLoadParity(
  ...
  const leftNodalGrouped = groupByKey(leftModel.nodalLoads ?? [], (load) => {
    if (!load.nodeKey) return undefined;
    const projectedNodeKey = leftNodeToRightNode.get(load.nodeKey);
    if (!projectedNodeKey) return undefined;
  ...
  for (const load of leftModel.nodalLoads ?? []) {
    if (!load.nodeKey || leftNodeToRightNode.has(load.nodeKey)) continue;
    mismatches.push({
      category: "load",
  ...
  for (const load of rightModel.nodalLoads ?? []) {
    ...
    if (!load.nodeKey || hasMatchedNode) continue;
    mismatches.push({
      category: "load",
      path: load.trace.sourcePath,
      leftValue: undefined,
  ...
  const nodalResult = compareGroupedLoads(
    leftNodalGrouped,
    rightNodalGrouped,
    (left, right) => {
      const equal = vectorsEqualWithinTolerance(
        left.vector as NodalLoadVector,
        right.vector as NodalLoadVector,
        loadTolerance,
      );
  ...
  const leftMembersByEndpoint = new Map<string, { nodeIKey: string; nodeJKey: string }>();
  ...
  const leftMemberGrouped = groupByKey(leftModel.memberLoads ?? [], (load) => {
    if (!load.endpointKey) return undefined;
    ...
    if (!projectedEndpoint) return undefined;
  ...
  for (const load of leftModel.memberLoads ?? []) {
    if (!load.endpointKey) continue;
    const endpoints = leftMembersByEndpoint.get(load.endpointKey);
    if (!endpoints) continue;
    const projectedEndpoint = projectEndpointKey(endpoints.nodeIKey, endpoints.nodeJKey, leftNodeToRightNode);
    if (projectedEndpoint) continue;
    mismatches.push({
      category: "load",
      path: load.trace.sourcePath,
  ...
  const memberResult = compareGroupedLoads(
    leftMemberGrouped,
    rightMemberGrouped,
    (left, right) => {
      const leftLoad = left as NormalizedMemberLoad;
      const rightLoad = right as NormalizedMemberLoad;
    ...
    (item) => ({
      coordinateSystem: (item as NormalizedMemberLoad).coordinateSystem,
      vector: item.vector,
    }),
  );
  ...
  const totalComparison = compareScalarWithTolerance(
  ...
  const unmatchedLeftNodalFromNode = (leftModel.nodalLoads ?? []).filter(
    (load) => load.nodeKey && !leftNodeToRightNode.has(load.nodeKey),
  ).length;
  const unmatchedRightNodalFromNode = (rightModel.nodalLoads ?? []).filter((load) => {
    if (!load.nodeKey) return false;
  ...
  const unmatchedLeftMemberFromEndpoint = (leftModel.memberLoads ?? []).filter((load) => {
    if (!load.endpointKey) return false;
    const endpoints = leftMembersByEndpoint.get(load.endpointKey);
