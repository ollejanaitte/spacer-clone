...
function fixityKey(support: NormalizedSupport): string {
...
function supportsByNodeKey(supports: NormalizedSupport[]): Map<string, NormalizedSupport[]> {
  const grouped = new Map<string, NormalizedSupport[]>();
...
export function compareSupportParity(
  ...
  const matchedRightNodes = new Set<string>();
  ...
  for (const match of nodeMatches) {
    ...
    if (leftSupports.length === 0 && rightSupports.length === 0) continue;
  ...
  for (const [nodeKey, supports] of rightByNode.entries()) {
    if (matchedRightNodes.has(nodeKey)) continue;
