...
export const PARITY_REPORT_ENVELOPE_SCHEMA_VERSION = "1.0.0";
...
function canonicalValueKey(value: unknown, seen: WeakSet<object>): string {
  if (value === undefined) {
    return "";
  }
...
function compareMismatches(left: ParityMismatch, right: ParityMismatch): number {
...
function compareDiagnostics(left: SemanticParityDiagnostic, right: SemanticParityDiagnostic): number {
  return (
    compareStrings(left.category, right.category)
    || compareStrings(left.path, right.path)
    || compareStrings(left.severity, right.severity)
...
function compareUnmatched(left: UnmatchedItem, right: UnmatchedItem): number {
  return (
...
function compareAmbiguities(left: AmbiguousMatch, right: AmbiguousMatch): number {
  return (
    compareStrings(left.category, right.category)
    || compareStrings(left.message, right.message)
...
function rejectUnsupportedValue(value: unknown): never {
...
function canonicalizeJsonValue(value: unknown, seen: WeakSet<object>): JsonSafeValue {
  if (value === null) {
    return null;
  }
  if (value === undefined) {
  ...
  if (valueType === "object") {
    ...
    const canonical: Record<string, JsonSafeValue> = {};
...
function normalizeGoldenSourceIndexPath(path: string, sourceId?: string): string {
  return path.replace(
    SOURCE_INDEX_PATH_PREFIX,
    (segment) => `${segment.split("/")[0]}/${goldenEntityIndexToken(sourceId)}`,
  );
}

function normalizeGoldenEmbeddedPaths(text: string, sourceId?: string): string {
  return text.replace(EMBEDDED_SOURCE_INDEX_PATH, (segment) => normalizeGoldenSourceIndexPath(segment, sourceId));
}

function normalizeGoldenUnmatchedKey(key: string, sourceId?: string): string {
  if (!sourceId) {
    return key;
  }
...
function goldenMismatchSourceIdHint(mismatch: ParityMismatch): string | undefined {
...
function goldenNormalizeMismatch(mismatch: ParityMismatch): ParityMismatch {
...
function goldenNormalizeDiagnostic(diagnostic: SemanticParityDiagnostic): SemanticParityDiagnostic {
  return {
    ...diagnostic,
...
function goldenNormalizeUnmatched(item: UnmatchedItem): UnmatchedItem {
  return {
    ...item,
...
function canonicalizeReportArrays(report: ParityReport): ParityReport {
  ...
  if (canonical.metrics.geometry.left.matchedMemberLengthDeltas) {
    canonical.metrics.geometry.left.matchedMemberLengthDeltas = sortNumbers(
      canonical.metrics.geometry.left.matchedMemberLengthDeltas,
    );
  }
  if (canonical.metrics.geometry.right.matchedMemberLengthDeltas) {
    canonical.metrics.geometry.right.matchedMemberLengthDeltas = sortNumbers(
      canonical.metrics.geometry.right.matchedMemberLengthDeltas,
    );
  }
  canonical.metrics.topology.left.connectedComponentSizes = sortNumbers(
    canonical.metrics.topology.left.connectedComponentSizes,
  );
  canonical.metrics.topology.right.connectedComponentSizes = sortNumbers(
    canonical.metrics.topology.right.connectedComponentSizes,
  );

  return canonical;
}

function canonicalizeReportArraysForGolden(report: ParityReport): ParityReport {
...
function buildEnvelope(
  ...
  const next: ParityReportEnvelope = {
...
export function createParityReportEnvelope(
  ...
  const envelope: ParityReportEnvelope = {
    schemaVersion: options.schemaVersion ?? PARITY_REPORT_ENVELOPE_SCHEMA_VERSION,
    sources: {
...
export function canonicalizeParityReportEnvelope(
  envelope: ParityReportEnvelope,
  options: CanonicalizeParityReportEnvelopeOptions = {},
): ParityReportEnvelope {
  const canonical = buildEnvelope(envelope, options);
  const seen = new WeakSet<object>();
  return canonicalizeJsonValue(canonical, seen) as ParityReportEnvelope;
}

function buildGoldenEnvelope(
  envelope: ParityReportEnvelope,
  options: CanonicalizeParityReportEnvelopeOptions = {},
): ParityReportEnvelope {
  const generatedAt = options.generatedAt ?? envelope.generatedAt;
...
export function canonicalizeParityReportEnvelopeForGolden(
  envelope: ParityReportEnvelope,
  options: CanonicalizeParityReportEnvelopeOptions = {},
): ParityReportEnvelope {
...
export function serializeParityReportEnvelopeForGolden(
  envelope: ParityReportEnvelope,
