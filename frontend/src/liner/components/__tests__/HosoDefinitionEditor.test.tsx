...
import { HosoDefinitionEditor } from "../HosoDefinitionEditor";
...
describe("HosoDefinitionEditor", () => {
  it("renders and adds a definition row", () => {
    ...
    act(() => {
      (document.querySelector("[data-testid=hoso-definition-add]") as HTMLButtonElement).click();
    });

    expect(draft.hosoDefinitions).toHaveLength(1);
    expect(document.querySelector("[data-testid^=hoso-definition-row-]")).not.toBeNull();
  });
});
