...
import { vec3, normalize3 } from "../vector";

export type MeasuredGridPreparationInput = {
  measuredGrid: MeasuredGridDraft;
  alignment: LinearAlignment;
  sourceRevision: string;
};

const DEFAULT_LOCAL_FRAME: LocalFrame = {
  tangent: { x: 1, y: 0, z: 0 },
  normal: { x: 0, y: 1, z: 0 },
  binormal: { x: 0, y: 0, z: 1 },
};

function rotate90Normal(tangent: { x: number; y: number }): { x: number; y: number } {
  return { x: -tangent.y, y: tangent.x };
}

function inferLocalFrameFromHclNeighbors(
  measuredGrid: MeasuredGridDraft,
  sectionId: string,
): LocalFrame {
  ...
  const hclBySection = new Map<string, { x: number; y: number; z: number; station: number }>();
  ...
  return {
    tangent: vec3(tangent2.x, tangent2.y, 0),
    normal: vec3(normal2.x, normal2.y, 0),
    binormal: vec3(0, 0, 1),
  };
}

function resolvePointRole(label: string, role?: string): GridPointRole[] {
...
export function generateMeasuredGridPoints(input: MeasuredGridPreparationInput): {
  ...
  const localFrameBySection = new Map<string, LocalFrame>();
  ...
  for (const point of measuredGrid.points) {
    ...
    gridPoints.push({
      id: gridPointId(alignment.linerModelId, section.sortIndex, line.sortIndex),
...
export function normalizeLocalFrame(frame: LocalFrame): LocalFrame {
  const tangent = normalize3(frame.tangent);
  const normal = normalize3(frame.normal);
  const binormal = normalize3(frame.binormal);
