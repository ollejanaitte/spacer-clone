...
export function BridgeAlignmentViewer({
  ...
  const stateRef = useRef({ alignment });
  stateRef.current = { alignment };
  ...
  useEffect(() => {
    ...
    function buildAxes(unit: number, labelSize: number): THREE.Group {
      ...
      g.add(createTextSprite("X", "#c0392b", labelSize));
      g.add(createTextSprite("Y", "#27ae60", labelSize));
      g.add(createTextSprite("Z", "#2980b9", labelSize));
      return g;
    }
    ...
    function rebuildGrid() {
      ...
      const align = stateRef.current.alignment;
      const extent = computeAlignmentExtent(align);
      const span = Math.max(extent.span, 10);
      ...
      grid.position.set(extent.centerX, extent.centerY, 0);
      gridLayer.add(grid);
    }
    ...
    function rebuildAxes() {
      ...
      const extent = computeAlignmentExtent(stateRef.current.alignment);
      const unit = Math.max(extent.span * 0.25, 2);
      const labelSize = Math.max(unit * 0.25, 0.5);
      const axes = buildAxes(unit, labelSize);
      axes.position.set(extent.centerX, extent.centerY, 0);
      scene.add(axes);
    }
    ...
    function rebuildCenterline() {
      ...
      const align = stateRef.current.alignment;
      if (align.points.length < 2) return;
      const pts = align.points.map((p) => new THREE.Vector3(p.x, p.y, p.z));
      ...
      align.points.forEach((p, idx) => {
        const isStart = idx === 0;
        const isEnd = idx === align.points.length - 1;
        ...
        if (showStationLabels) {
          ...
          sprite.position.set(p.x, p.y, p.z + 0.6);
    ...
    function fitToBounds() {
      ...
      for (const p of align.points) box.expandByPoint(new THREE.Vector3(p.x, p.y, p.z));
